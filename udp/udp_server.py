...
class UdpServer:
    ...
    def start_server(self):
        while True:
            ...
            response, address = self.receive_udp_message(self.host, self.port)

            client_host = address[0]
            client_port = address[1]

            self.send_udp_message(response.as_bytes(), client_host, client_port)
    ...
    def receive_udp_message(self,host, port):
            ...
            try:
                ...
                response = UdpMessage(response_is_request, response_request_type, response_identifier, len(response_message), response_message)
                ...
                response = UdpMessage(response_is_request, response_request_type, response_identifier)
    ...
    def send_udp_message(self, message, host, port):
        ...
        try:
            ...
            print("Mensagem enviada com sucesso para ", host, ":", port)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    server = UdpServer(IP_SERVIDOR, PORTA_SERVIDOR)
    server.start_server()